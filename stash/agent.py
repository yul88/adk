...
class Stash(Agent):
    ...
    def __init__(self):
        ...
        # Use object.__setattr__ to bypass Pydantic validation if needed, but simple assignment should work after init if PrivateAttr
        ...
        # Initialize service and override the internal attribute
    ...
    async def signup(self, username: str, password: str, tool_context: Optional[ToolContext] = None) -> str:
        ...
        """
        # pymongo is sync, but strictly it blocks the loop. 
        # For this scale/context, it's acceptable (Agent Engine typically low concurrency per instance or scales instances).
    ...
    async def stash_link_smart(self, url: str, tool_context: Optional[ToolContext] = None) -> str:
        ...
        try:
            ...
            # GenAI client is sync? 
            # If Client(vertexai=True), generate_content might have async version?
            # Standard python client is typically sync unless .aio used.
            # We'll use sync call here for simplicity unless it errors.
...
# Instantiate the agent for ADK CLI
root_agent = Stash()
